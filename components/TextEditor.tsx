...
export default function TextEditor({ visible, onClose, textElementId }: TextEditorProps) {
  const { elements, updateElement, addElement } = useDesignStore();
  ...
  const textElement = textElementId === 'new' ? null : elements.find(el => el.id === textElementId) as TextElement | undefined;
  ...
  // Update width live as text or fontSize changes
  useEffect(() => {
    if (textElementId && textElementId !== 'new' && text.trim() && textElement) {
      const width = estimateTextWidth(text, fontSize);
      updateElement(textElementId as string, { width });
    }
  }, [text, fontSize, textElementId, updateElement, textElement]);
  ...
  const handleSave = () => {
    ...
    if (textElementId === 'new') {
      ...
      const newText: TextElement = {
        ...
        y: 100,
        width: estimateTextWidth(text.trim(), fontSize),
        height: fontSize,
        text: text.trim(),
        fontSize,
        fontFamily,
  ...
  if (!textElement && textElementId !== 'new') return null;
